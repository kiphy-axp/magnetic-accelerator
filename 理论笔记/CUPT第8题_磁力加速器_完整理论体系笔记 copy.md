...
【本文推导】为了把“一个磁体对附近先被吸引、再越过势垒、最后离开”的定性图像接到定量分析，可以先只看相邻两对轨道磁体之间的一个局部周期单元

$$
x\in[x_j,x_j+d_m].
$$

【本文推导】这里的“单个周期”不是说整条 V 形轨道严格周期，而是指沿 $x$ 方向相邻磁体对之间的局部分析单元。V 形/楔形横向间距、入口/出口和非等价极性会让每个单元的慢变偏置不同。

在这个局部单元内，把有效势能写成周期项加慢变偏置：

$$
U_{\text{eff}}(x)
=U_{\text{per},j}(x)+U_{\text{bias},j}(x),
\qquad
U_{\text{per},j}(x+d_m)\approx U_{\text{per},j}(x).
$$

【本文推导】单个周期内可按势能斜率定性分成三段：

| 周期内区间 | 定性运动 | 定量判据 |
|---|---|---|
| 进入段 | 车辆被前方磁体对吸引，速度可能上升 | $-dU_{\text{eff}}/dx>F_{\text{loss}}$ |
| 过顶段 | 车辆越过局部势垒，最容易卡住或回滚 | $K(x)+U_{\text{eff}}(x)>U_{\text{barrier},j}$ |
| 离开段 | 车辆脱离该对磁体影响，可能被后方拉回或进入下一周期 | $\Delta K_j=-\Delta U_j-\Delta E_{\text{loss},j}$ |

其中第 $j$ 个周期的净势能变化定义为

$$
\Delta U_j
=U_{\text{eff}}(x_j+d_m)-U_{\text{eff}}(x_j),
$$
...
$$
U_{\text{barrier},j}
=\max_{x\in[x_j,x_j+d_m]}U_{\text{eff}}(x).
$$

因此过顶段的更完整条件应写成

$$
K(x)+U_{\text{eff}}(x)>U_{\text{barrier},j}
$$

或等价地说，车辆剩余机械能必须足以越过该局部势垒。

对应的能量账为

$$
\Delta K_j
=-\Delta U_j-\Delta E_{\text{loss},j}.
$$

【本文推导】如果轨道在该周期前后完全等价，则 $\Delta U_j\approx0$，单周期只能出现“先加速后减速”的局部速度变化，不能提供净动能增长。若 V 形/楔形横向间距、入口/出口边界或非等价极性使 $U_{\text{bias},j}$ 下降，则
...
【实验待定】单周期定性分段必须由实测 $U_{\text{eff}}(x)$ 或 $F_{\text{drive}}(x)$ 验证。不能只凭肉眼看到“靠近磁体时变快”就断言整个周期净加速；必须比较周期起点和终点的 $K$、$U_{\text{eff}}$ 与损耗。
